...
var ReferencePanel = Component.extend({
  ...
  'init' : function(node, options) {
    ...
    this.endpointsList = new ListComponent({
      ...
      'className' : 'endpoints',
      'itemTemplate' : function(item) {
        return $('<li></li>').text(item.method + " /v" + item.version + (item.path_labeled || "?"));
      }
    });
  ...
  'onChange' : function(value, oldValue) {
    ...
    $.each(value || {}, function(version, endpoints) {
      $.each(endpoints, function(idx, endpoint) {
        endpoint.version = version;
      });
    });

    $.each(oldValue || {}, function(version, versionEndpoints) {
      if (!value[version]) {
        value[version] = versionEndpoints;
      }
    });
...
ReferencePanel.prototype.getEndpoints = function() {
  ...
  $.each(this.getValue(), function(v, endpoints) {
    flattened = flattened.concat(endpoints);
  });

  return this.options.filter(flattened);
};
