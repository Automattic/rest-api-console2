...
ReferencePanel.prototype.partitionedEndpoints = function() {
  ...
  if(this.options.history && this.options.history[apiName] && this.options.history[apiName].length > 0) {
    all = [{history: true, label: 'Recent'}].concat(this.options.history[apiName]).concat(all);
  }
  return all;
};
...
ReferencePanel.prototype.addHistory = function(endpoint) {
  var apiName = this.api.name;
  if (!this.options.history) this.options.history = {};
  if (!this.options.history[apiName]) this.options.history[apiName] = [];


  this.options.history[apiName] = this.options.history[apiName].filter(function(item) {
    return !(item.path_labeled == endpoint.path_labeled && item.method == endpoint.method);
  }).slice(0, 10);

  this.options.history[apiName].unshift(endpoint);
