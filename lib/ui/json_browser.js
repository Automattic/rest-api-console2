...
var JsonBrowser = Component.extend({
  ...
  'init' : function() {
    this.node.on('click', '[data-json]', (function(e) {

      var $node = $(e.currentTarget),
          expanded = render(JSON.parse(atob($node.data('json'))), true);
...
function renderObject(json, recursive) {
  ...
  for(var key in json) {
    list.append(buildRow('"' + key + '"', json[key], recursive));
  }
...
function buildRow(key, value, recursive) {
  ...
  row.append($("<span></span>").addClass('key').append($('<code></code>').text(key + ': ')))
