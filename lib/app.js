...
var authPanel = new ui.AuthPanel('#auth', {
    ...
    queryBuilder = new ui.ParamBuilder($('#query'), {
      'title':'Query',
      'formatOptions': [ui.ParamFormatter.queryString()],
    }),
    bodyBuilder = new ui.ParamBuilder($('#body'), {
      'title':'Body',
      'formatOptions': [ui.ParamFormatter.queryString(), ui.ParamFormatter.json()],
    }),
    tip = new ui.Tip('#tip'),
    ...
    selectEndpoint = function(endpoint) {
      ...
      queryBuilder.setFormatOptions([ui.ParamFormatter.form(), ui.ParamFormatter.query()]);
      queryBuilder.setValue(endpoint.request.query);

      bodyBuilder.setFormatOptions([ui.ParamFormatter.form(), ui.ParamFormatter.query(), ui.ParamFormatter.json()]);
...
pathField.on('reset', function(){
  ...
  queryBuilder.setFormatOptions([ui.ParamFormatter.query()]);
  queryBuilder.reset();

  bodyBuilder.setFormatOptions([ui.ParamFormatter.query(), ui.ParamFormatter.json()]);
  bodyBuilder.reset();
  tip.reset();
});
