...
var
    ...
    referencePanel = new ui.ReferencePanel({
      ...
      'onLoadEndpoints': function(version, onEndpoints) {
        ...
        var endPointsUrl = apiSelector.getValue() === 'WP.COM'
          ? "https://public-api.wordpress.com/rest/v" + version +"/help"
        ...
        $.ajax({
          ...
          success: function(data) {
            var endpoints;
            if (apiSelector.getValue() === 'WP.COM') {
              endpoints = $.map(data, function(endpoint, index) {
                endpoint.version = version;
                return endpoint;
              });
            } else {
              endpoints = WPApi.parseEndpoints(data);
            }
            onEndpoints(null, endpoints);
          }
        });
      }
    }),
    ...
    pathField = new ui.PathField($('#path'), {
      ...
      'itemTemplate': function(item) {
        ...
        var listItem = $("<li></li>"),
            pathText = item.version
                ? '/v' + item.version + item.path_labeled
                : item.path_labeled,
            method = $("<span></span>").text(item.method).appendTo(listItem),
            path = $("<code></code>").text(pathText).appendTo(listItem.append(" ")),
    ...
    selectEndpoint = function(endpoint) {
        endpointPath = new path.Path(
            endpoint.version
                ? '/v' + endpoint.version + endpoint.path_labeled
                : endpoint.path_labeled,
